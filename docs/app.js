(() => {
  ...
  function initHeroBannerSlideshow() {
    ...
    const intervalMs = reduceMotion ? 6000 : 4200;
  ...
  function initScrollFootprintPath() {
    const host = document.getElementById("scrollFootprintPath");
    if (!host) return;
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      fp.className = "path-footprint";
      fp.style.top = `${(i / (count - 1)) * 96}%`;
      fp.style.left = i % 2 === 0 ? "6px" : "13px";
      fp.style.transform = i % 2 === 0 ? "rotate(-20deg)" : "rotate(14deg)";
      host.appendChild(fp);
    }

    const marks = [...host.querySelectorAll(".path-footprint")];
    const paint = () => {
      const doc = document.documentElement;
      const scrollMax = doc.scrollHeight - window.innerHeight;
      const progress = scrollMax > 0 ? window.scrollY / scrollMax : 0;
      const active = Math.floor(progress * (count - 1));
      marks.forEach((mark, i) => mark.classList.toggle("active", i <= active));
    };
